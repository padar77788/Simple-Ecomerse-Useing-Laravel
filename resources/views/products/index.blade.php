                    ...
                    @foreach($products as $key => $product)
                        <tr>
                            ...
                            <td>
                             ...
                             <td>
                                <label class="switch">
                                    <input onchange="update_hot(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->hot == 1) echo "checked";?> >
                                    <span class="slider round"></span>
                                </label>
                            </td>
                             <td>
                                <label class="switch">
                                    <input onchange="update_instagram(this)" value="{{ $product->id }}" type="checkbox" <?php if($product->instagram == 1) echo "checked";?> >
                                    <span class="slider round"></span>
                                </label>
                            </td>
                            <td>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_instagram(el){
            ...
            $.post('{{ route('products.instagram') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Instagram products updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
        ...
        function update_hot(el){
            ...
            $.post('{{ route('products.hot') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    showAlert('success', 'Hot products updated successfully');
                }
                else{
                    showAlert('danger', 'Something went wrong');
                }
